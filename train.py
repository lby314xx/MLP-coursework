...
parser.add_argument("--use_sk", action="store_true", help="Use SKLayer?")
parser.add_argument("--batchSize", type=int, default=256, help="training batch size")
parser.add_argument("--rate", default=4, type=int, help="upscale rate, Default: n=2")
...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        train(training_data_loader, optimizer, model, criterion, epoch, cuda)


def train(training_data_loader, optimizer, model, criterion, epoch, cuda):
    print("epoch =", epoch, "lr =", optimizer.param_groups[0]["lr"])

    model.train()
    print(len(training_data_loader))
    for iteration, batch in enumerate(training_data_loader, 1):  # 1为起始下标
        ...
        print(iteration)
        # print(batch[int(math.sqrt(opt.rate))].size())
        if opt.cuda:
            input = input.cuda()
            label = label.cuda()
        else:
        ...
        if iteration % 10 == 0:
            print("===> Epoch[{}]({}/{}): Loss: {:.6f}".format(epoch, iteration, len(training_data_loader),
                                                               loss.data))
            logger.add_scalar('loss', loss.data, len(training_data_loader) * epoch + iteration)

    save_checkpoint(model, opt.rate, epoch, cuda)
